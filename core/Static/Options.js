...
const OptionsApp = {
    setup() {
        ...
        const showMessage = (type, content, duration = 3000) => {
            // 显示新消息
            const msgElement = motyf[type](content);

            // 除了error类型，其他类型的消息在指定时间后自动关闭
            if (type !== 'error' && duration > 0) {
                setTimeout(() => {
                    if (msgElement && msgElement.classList) {
                        // 添加退出动画类
                        msgElement.classList.add('motym-out');
                        // 延迟删除元素
                        setTimeout(() => {
        ...
        const saveSettings = async () => {
            ...
            // 显示加载消息
            let loadingMsg = showMessage('info', '保存中...', 0); // 0表示不自动关闭
            ...
            try {
                ...
                if (!apiUrl) {
                    showMessage('error', 'API URL未配置，无法保存设置');
                ...
                // 关闭加载消息
                if (loadingMsg && loadingMsg.classList) {
                    loadingMsg.classList.add('motym-out');
                    setTimeout(() => {
                ...
                // 修改判断逻辑
                if (result.code === 200) {
                    showMessage('success', result.data.message || '设置已保存');
                } else {
                    showMessage('error', result.data.message || result.message || '保存失败');
                }
            } catch (error) {
                // 关闭加载消息
                if (loadingMsg && loadingMsg.classList) {
